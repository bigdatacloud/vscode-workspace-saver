...
Task này tồn tại để tạo vùng phủ chạy thật cho ba tầng không có unit test
(`src/terminal/manager.ts`, `src/workspace/manager.ts`, `src/ui/*`). Vì vậy mỗi test PHẢI gọi
lớp thật (`TerminalManager`, lệnh do `activate()` đăng ký, view do `package.json` contribute)
thay vì gọi thẳng API trần của `vscode` rồi assert `true` — một test như vậy báo cáo vùng phủ
không tồn tại, còn tệ hơn không có test. Test biên dịch cùng `src/` (tsconfig include cả hai,
cùng ra `out/`) nên import trực tiếp lớp thật từ `src/terminal/manager.ts` được.
...
suite('AI Workspace extension', () => {
  ...
  test('đăng ký đủ 8 lệnh của MVP và activate() đã chạy xong', async () => {
    ...
    // bấm cả nên await sẽ treo tới hết timeout. 'aiWorkspace.closeWorkspace' thì khi chưa có
    // workspace mở sẽ return ngay lập tức (xem WorkspaceManager.close()), vẫn chứng minh được
    // handler đã đăng ký và chạy được mà không cần tương tác UI.
...
Expected: 5 test PASS trong cửa sổ VS Code test.
