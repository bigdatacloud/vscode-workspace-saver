...
| AI Workspace: Voice into terminal (start / stop) | `aiWorkspace.voiceToggle` | `Ctrl+Shift+Y` (terminal or AI Workspaces view focused) — shows a **picker**: local (VS Code ≥ 1.131 built-in Nemotron, offline, Vietnamese supported) or cloud (OpenAI gpt-4o-mini-transcribe / gpt-4o-transcribe / whisper-1, Gemini flash-lite / 3.5 / 3.7); Enter = default, picking another entry uses it now and saves it (`aiWorkspace.voice.model`); disable the prompt with `aiWorkspace.voice.askOnStart`. Local: dictation into a scratch tab, model **pre-warmed on window open** (`warmUpOnStartup`, mic on for ~40 s once) so you can speak immediately. Cloud: the extension records via a WinMM helper (Windows), speak immediately, stop sends the audio and types the text back (Gemini flash-lite ~2 s). `Ctrl+Shift+Y` again stops; text (punctuation kept, one line) lands in the terminal input, **Enter is never sent for you**. The `AI Workspace — Giọng nói` output channel logs every step. On first run the extension adds this command to `terminal.integrated.commandsToSkipShell` (with a notice) so the shell does not swallow the key |
| AI Workspace: Set voice API key (OpenAI / Gemini) | `aiWorkspace.voiceSetApiKey` | Command Palette or from the picker — key goes to VS Code SecretStorage, never to settings; empty input deletes it |
| AI Workspace: Select voice microphone | `aiWorkspace.voiceSelectMicrophone` | Command Palette — opens VS Code's microphone picker for dictation. Use it when dictation ends with no text: Chromium's "default" microphone can be a silent virtual device (log shows `reason=noTranscript`) |
| AI Workspace: Duplicate terminal into its own worktree | `aiWorkspace.duplicateTerminal` | Terminal context menu — creates a new terminal with the same role and agent kind in its **own worktree and git branch** (`<task>-<role>`). For a Claude terminal with a known session id you can pick **continue this conversation** (`claude --resume … --fork-session`): the copy carries the full context without colliding with the original (two processes resuming one session write to the same conversation file). The old session id is **never** copied to the duplicate; a duplicated Codex terminal always opens a fresh session (keeping `--yolo` if the original had it) |
