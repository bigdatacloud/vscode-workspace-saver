...
| AI Workspace: Voice into terminal (start / stop) | `aiWorkspace.voiceToggle` | `Ctrl+Shift+Y` (terminal or AI Workspaces view focused) — starts VS Code's built-in dictation (≥ 1.131, offline Nemotron, Vietnamese supported) into a **scratch editor tab**, shows a LISTENING indicator in the status bar plus a progress toast; `Ctrl+Shift+Y` again stops: the scratch tab closes and the text (punctuation kept, joined to one line) is typed into the most recent terminal's input, **Enter is never sent for you**. Wait until the scratch tab stops saying the model is being prepared (20–30 s per start) before speaking. The `AI Workspace — Giọng nói` output channel logs every step. On first run the extension adds this command to `terminal.integrated.commandsToSkipShell` (with a notice) so the shell does not swallow the key |
