...
| AI Workspace: Voice into terminal (start / stop) | `aiWorkspace.voiceToggle` | `Ctrl+Shift+Y` (terminal or AI Workspaces view focused) — starts VS Code's built-in dictation (≥ 1.131, offline Nemotron, Vietnamese supported) into the most recent terminal, shows a LISTENING indicator in the status bar plus a progress toast; `Ctrl+Shift+Y` again stops. Text lands in the terminal input, **Enter is never sent for you**. On first run the extension adds this command to `terminal.integrated.commandsToSkipShell` (with a notice) so the shell does not swallow the key |
