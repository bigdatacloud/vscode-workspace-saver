...
| AI Workspace: Nghe giọng nói vào terminal (bật / dừng) | `aiWorkspace.voiceToggle` | `Ctrl+Shift+Y` (khi terminal hoặc cây AI Workspaces đang focus) — hiện **bảng chọn**: trên máy (Nemotron của VS Code ≥ 1.131, offline, có tiếng Việt) hay cloud (OpenAI gpt-4o-mini-transcribe / gpt-4o-transcribe / whisper-1, Gemini flash-lite / 3.5 / 3.7); Enter = mặc định, chọn mục khác là dùng luôn và lưu (`aiWorkspace.voice.model`); tắt hỏi bằng `aiWorkspace.voice.askOnStart`. Trên máy: dictation vào tab nháp, model được **làm nóng lúc mở cửa sổ** (`warmUpOnStartup`, micro sáng ~40 s một lần) nên bấm là nói ngay. Cloud: extension ghi âm bằng helper WinMM (Windows), bấm là nói, dừng thì gửi và đưa chữ về (Gemini flash-lite ~2 s). Bấm `Ctrl+Shift+Y` lần nữa để dừng; chữ (giữ dấu câu, gộp một dòng) vào ô nhập terminal, **không tự gửi Enter**. Kênh Output `AI Workspace — Giọng nói` ghi từng bước. Lần đầu extension tự thêm lệnh này vào `terminal.integrated.commandsToSkipShell` (có báo) để shell không nuốt phím. Nếu chỉ thấy "Waiting for second key of chord…" thì extension khác đang giữ phím này làm phím đầu tổ hợp — gán phím khác cho `aiWorkspace.voiceToggle` trong Keyboard Shortcuts |
| AI Workspace: Nhập API key nghe giọng nói (OpenAI / Gemini) | `aiWorkspace.voiceSetApiKey` | Command Palette hoặc trong bảng chọn — key vào kho bí mật của VS Code (SecretStorage), không vào settings; để trống là xoá |
| AI Workspace: Chọn micro nghe giọng nói | `aiWorkspace.voiceSelectMicrophone` | Command Palette — mở bảng chọn micro của VS Code. Dùng khi nghe xong không ra chữ: micro "mặc định" theo Chromium có thể là thiết bị ảo câm (log ghi `reason=noTranscript`) |
| AI Workspace: Nhân bản terminal ra worktree riêng | `aiWorkspace.duplicateTerminal` | Context menu terminal — tạo một terminal mới cùng vai, cùng loại agent, trong **worktree + nhánh git riêng** (`<việc>-<vai>`). Với Claude đã có id phiên, chọn được **nối tiếp hội thoại** (`claude --resume … --fork-session`): bản sao mang toàn bộ bối cảnh mà **không** giẫm lên bản gốc (hai tiến trình cùng `--resume` một phiên là cùng ghi một file hội thoại). Id phiên cũ **không bao giờ** được chép sang bản sao; Codex bản sao luôn mở phiên mới (giữ `--yolo` nếu bản gốc có) |
