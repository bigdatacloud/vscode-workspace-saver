...
| AI Workspace: Nghe giọng nói vào terminal (bật / dừng) | `aiWorkspace.voiceToggle` | `Ctrl+E` (khi terminal hoặc cây AI Workspaces đang focus) — bật dictation tích hợp của VS Code (≥ 1.131, offline, có tiếng Việt) vào terminal hoạt động gần nhất, hiện chỉ báo ĐANG NGHE ở thanh trạng thái + thông báo tiến trình; bấm `Ctrl+E` lần nữa để dừng. Chữ nằm trong ô nhập, **không tự gửi Enter**. Lần đầu extension tự thêm lệnh này vào `terminal.integrated.commandsToSkipShell` (có báo) để shell không nuốt phím. Nếu chỉ thấy "Waiting for second key of chord…" thì extension khác (thường là SQLTools) đang giữ Ctrl+E làm phím đầu tổ hợp — đặt `sqltools.disableChordKeybindings: true` hoặc gán phím khác |
