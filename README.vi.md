...
| AI Workspace: Nghe giọng nói vào terminal (bật / dừng) | `aiWorkspace.voiceToggle` | `Ctrl+Shift+Y` (khi terminal hoặc cây AI Workspaces đang focus) — bật dictation tích hợp của VS Code (≥ 1.131, offline, có tiếng Việt) vào một **tab nháp**, hiện chỉ báo ĐANG NGHE ở thanh trạng thái + thông báo tiến trình; bấm `Ctrl+Shift+Y` lần nữa để dừng: tab nháp đóng, chữ (giữ dấu câu, gộp một dòng) được đưa vào ô nhập của terminal hoạt động gần nhất, **không tự gửi Enter**. Chờ tab nháp hết báo "đang chuẩn bị model" (20–30 giây mỗi lần bắt đầu) rồi mới nói. Kênh Output `AI Workspace — Giọng nói` ghi từng bước. Lần đầu extension tự thêm lệnh này vào `terminal.integrated.commandsToSkipShell` (có báo) để shell không nuốt phím. Nếu chỉ thấy "Waiting for second key of chord…" thì extension khác đang giữ phím này làm phím đầu tổ hợp — gán phím khác cho `aiWorkspace.voiceToggle` trong Keyboard Shortcuts |
